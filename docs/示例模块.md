...
- `title`: 菜单标题
- `icon`: 菜单图标（Heroicons）
- `order`: 排序权重，越小越靠前，默认 999
- `children`: 子菜单项数组
- 子菜单项支持 `external`（外部链接）、`show`（显示/隐藏）、`order`（排序）等配置
...
![Markdown 编辑器](assets/example-markdown-editor.png)

### Markdown 渲染 (Markdown Preview)

展示了基于 `react-markdown` + `github-markdown-css` 的 Markdown 内容渲染效果。

- **路径**: `/examples/markdown-render`
- **文件**: `src/modules/examples/MarkdownPreview.tsx`
- **主要功能**:
    - 内置丰富示例内容（标题、列表、代码高亮、表格、图片等）。
    - 支持 GFM 语法（表格、任务列表、删除线）。
    - 支持 HTML 标签渲染（rehype-raw）。
    - 编辑/预览双 Tab 切换，可编辑 Markdown 并实时查看渲染效果。

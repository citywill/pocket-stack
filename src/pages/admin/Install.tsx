      ...
      <Card className="w-full max-w-2xl rounded-2xl border-none shadow-lg bg-white/50 backdrop-blur-sm dark:bg-neutral-900/50">
        <CardHeader className="text-center">
          <div className="mx-auto w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-2xl flex items-center justify-center mb-4">
            <HugeiconsIcon icon={DatabaseImportIcon} className="w-8 h-8 text-blue-600" />
          </div>
        ...
        <CardContent className="space-y-6">
          {isInstalled && !installing && (
            <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-xl border border-green-100 dark:border-green-800">
              <div className="flex gap-3">
                <HugeiconsIcon icon={InformationCircleIcon} className="w-5 h-5 text-green-600 shrink-0 mt-0.5" />
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-800">
            <div className="flex gap-3">
              <HugeiconsIcon icon={AlertCircleIcon} className="w-5 h-5 text-blue-600 shrink-0 mt-0.5" />
        ...
        <CardFooter className="flex flex-col gap-3">
          ...
          >
            {checking ? '正在检查安装状态...' :
              installing ? '正在安装...' :
                isInstalled ? '更新系统结构' : '开始安装并导入'}
            {!installing && !checking && !isInstalled && <HugeiconsIcon icon={ArrowRight01Icon} className="ml-2 w-5 h-5" />}
          </Button>
